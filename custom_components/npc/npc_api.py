"""API client for EVN VN (NPC, HN, CPC, SPC, HCMC) - ĐÃ CẬP NHẬT SPC"""
...
class EVNAPI:
    ...
    def __init__(self, hass, region: str, username: str, password: str, customer_id: str):
        ...
        self.device_id: Optional[str] = "949A66CA-C0DD-4ECF-9624-A918B13F6FC8"  # Device ID cho SPC
        self.spc_secret: Optional[str] = None  # Secret từ SPC login
        ...
        try:
            ...
            if self.region == "SPC":
                ...
                headers = {
                    ...
                    "user-agent": "evnapp/113 CFNetwork/3826.600.41 Darwin/24.6.0",
                ...
                payload = {
                    ...
                    "strDeviceID": self.device_id,
                }
                
                async with session.post(url, json=payload, headers=headers, ssl=False, timeout=self._timeout) as resp:
                    if resp.status != 200:
                        _LOGGER.debug(f"SPC Login failed with status {resp.status}")
                        return False
                    ...
                    if not token:
                        _LOGGER.debug("No token in SPC login response")
                        return False
                    
                    self.access_token = token
                    self.spc_secret = data.get("secret")
                    self.ma_khang = data.get("maKH")
                    self.ma_dviqly = data.get("maDonVi")
                    
                    _LOGGER.info(
                        f"SPC Login successful for {self.customer_id}, "
                        f"maKH={self.ma_khang}, maDonVi={self.ma_dviqly}"
                    )
                    return True
            
            # ===== Các vùng khác: Login chung =====
            else:
                payload = {
                ...
                async with session.post(LOGIN_URL, json=payload, headers=headers, ssl=False, timeout=self._timeout) as resp:
                    if resp.status != 200:
                    ...
                    if self.region == "HN":
                        ...
                        self.ma_dviqly = ma_kh_login[:6] if len(ma_kh_login) >= 6 else ma_kh_login
                        self.ma_ddo = ma_kh_login
                    else:
                        cid = self.customer_id or ""
                        self.ma_dviqly = cid[:6] if len(cid) >= 6 else cid
                        self.ma_ddo = cid
    ...
    async def _login_hcmc_session(self) -> bool:
        ...
        try:
            ...
            async with session.post(hcmc_login_url, data=payload, headers=headers, ssl=False, timeout=self._timeout) as resp:
                if resp.status != 200:
    ...
    async def _switch_account(self, token: str) -> bool:
        ...
        try:
            ...
            async with session.get(switch_url, headers=headers, ssl=False, timeout=self._timeout) as resp:
                if resp.status != 200:
                ...
                if self.region == "HN":
                    ...
                    self.ma_dviqly = ma_kh_str[:6] if len(ma_kh_str) >= 6 else ma_kh_str
    ...
    async def get_chisongay(
        ...
        try:
            ...
            if self.region == "HCMC":
                ...
                async with session.post(url, data=payload, headers=headers, ssl=False, timeout=self._timeout) as resp:
                    if resp.status != 200:
            ...
            # ===== SPC: CẬP NHẬT HOÀN TOÀN =====
                ...
                # strMaDiemDo = customer_id + "001"
                ma_diem_do = f"{self.customer_id}001"
                
                url = f"{self.base_url}/api/NghiepVu/LayThongTinSanLuongTheoNgay_v2"
                params = {
                    "strMaDiemDo": ma_diem_do,
                    "strFromDate": from_date_str,
                    "strToDate": to_date_str,
                }
                
                # Cập nhật headers mới cho SPC
                headers = {
                    "accept": "application/json",
                    "user-agent": "evnapp/113 CFNetwork/3826.600.41 Darwin/24.6.0",
                ...
                _LOGGER.debug(f"get_chisongay (SPC): URL={url}, params={params}")
                
                async with session.get(url, params=params, headers=headers, ssl=False, timeout=self._timeout) as resp:
                    if resp.status == 401:
                        # Token expired, try login again
                        if await self.login():
                            headers["authorization"] = f"Bearer {self.access_token}"
                            async with session.get(url, params=params, headers=headers, ssl=False, timeout=self._timeout) as retry_resp:
                    ...
                    if resp.status != 200:
                        error_text = await resp.text()
                        _LOGGER.debug(f"get_chisongay failed with status {resp.status}, response: {error_text[:500]}")
                        return None
        ...
        try:
            ...
            if self.region == "HCMC" or self.region == "SPC":
                ...
                if self.region == "HCMC":
                    ...
                    chi_so_cu = float(first_record.get("CHISO", 0) or 0)
                    chi_so_moi = float(last_record.get("CHISO", 0) or 0)
                    diff = float(chi_so_moi - chi_so_cu)
                    chi_so_thang = float("{:.2f}".format(diff))
                    
                    str_time_first = first_record.get("NGAY", "")
                    str_time_last = last_record.get("NGAY", "")
                ...
                async with session.post(url, json=payload, headers=headers, ssl=False, timeout=self._timeout) as resp:
                    if resp.status == 401:
                        if await self.login():
                            headers["authorization"] = f"Bearer {self.access_token}"
                            async with session.post(url, json=payload, headers=headers, ssl=False, timeout=self._timeout) as retry_resp:
                                if retry_resp.status != 200:
    ...
    async def get_hoadon(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # SPC dùng endpoint và format riêng
                ...
                _LOGGER.debug(f"get_hoadon (SPC): URL={url}, params={params}")
                ...
                async with session.get(url, params=params, headers=headers, ssl=False, timeout=self._timeout) as resp:
                    ...
                    if resp.status != 200:
                        error_text = await resp.text()
                        _LOGGER.debug(f"get_hoadon failed with status {resp.status}, response: {error_text[:500]}")
                        return None
                ...
                async with session.post(url, headers=headers, ssl=False, timeout=self._timeout) as resp:
                    if resp.status == 401:
                        if await self.login():
                            headers["authorization"] = f"Bearer {self.access_token}"
                            async with session.post(url, headers=headers, ssl=False, timeout=self._timeout) as retry_resp:
                                if retry_resp.status != 200:
    ...
    async def get_ngungcapdien(
        ...
        try:
            ...
            if self.region == "SPC":
                ...
                _LOGGER.debug(f"get_ngungcapdien (SPC): URL={url}, params={params}")
                ...
                async with session.get(url, params=params, headers=headers, ssl=False, timeout=self._timeout) as resp:
                    ...
                    if resp.status != 200:
                        error_text = await resp.text()
                        _LOGGER.debug(f"get_ngungcapdien failed with status {resp.status}, response: {error_text[:500]}")
                        return None
